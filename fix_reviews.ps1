$content = Get-Content "resources\js\Pages\Home\Activity\MyReviews.vue" -Raw
...
$componentHeader = @"
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 text-[#1D1D1F]">
                ...
                <div v-if="isComponent" class="flex justify-between items-center mb-6">
                    <h2 class="text-xl font-bold text-[#1D1D1F]">Ulasan & Penilaian Saya</h2>
                </div>
"@
$content = $content.Replace($mobileTopFilter, $componentHeader)

Set-Content "resources\js\Pages\Home\Activity\MyReviews.vue" $content
