...
<main class="relative min-h-screen w-full bg-cover bg-center bg-no-repeat font-sans antialiased text-slate-800"
  ...
  <div class="relative mx-auto flex min-h-screen w-full max-w-7xl items-center justify-between px-6 py-12 md:px-12">
    ...
    <!-- FORM CARD REGISTER LEBIH LEBAR KANAN (2 KOLOM SESUAI GAMBAR) -->
    <div class="mx-auto w-full max-w-[640px] rounded-2xl bg-white p-8 shadow-2xl md:mx-0">
      <!-- Logo Brand -->
      <div class="flex flex-col items-center justify-center">
        ...
        <h2 class="mt-4 text-xl font-bold text-[#fec107] tracking-wide">Register</h2>
        <div class="mt-4 h-[1px] w-full bg-slate-100"></div>
      </div>

      <!-- Form Inputs dengan Layout Grid 2 Kolom -->
      <form action="{{ route('register') }}" method="POST" class="mt-6">
        ...
        <div class="grid gap-x-8 gap-y-5 sm:grid-cols-2">
          ...
          <!-- Kanan: Kata Sandi -->
          <div class="space-y-1">
            <label class="text-xs font-semibold text-slate-800">Kata Sandi</label>
            <div
              class="relative border-b border-slate-300 focus-within:border-[#fec107] transition pb-1.5 flex items-center">
              <span class="absolute left-0 text-amber-500/70 text-sm">🔒</span>
          ...
          <!-- Kiri: Username -->
          <div class="space-y-1">
            <label class="text-xs font-semibold text-slate-800">Username</label>
            <div
              class="relative border-b border-slate-300 focus-within:border-[#fec107] transition pb-1.5 flex items-center">
              <span class="absolute left-0 text-amber-500/70 text-sm">👤</span>
              <input type="text" name="username" placeholder="jonh doe" required
                class="w-full bg-transparent pl-6 text-xs text-slate-700 outline-none placeholder:text-slate-300">
            </div>
          </div>

          <!-- Kanan: Ulangi Kata Sandi -->
          <div class="space-y-1">
            <label class="text-xs font-semibold text-slate-800">Ulangi Kata Sandi</label>
            <div
              class="relative border-b border-slate-300 focus-within:border-[#fec107] transition pb-1.5 flex items-center">
              <span class="absolute left-0 text-amber-500/70 text-sm">🔒</span>
          ...
          <!-- Kiri: Nomor HP -->
          <div class="space-y-1">
            <label class="text-xs font-semibold text-slate-800">Nomor HP</label>
            <div
              class="relative border-b border-slate-300 focus-within:border-[#fec107] transition pb-1.5 flex items-center">
              <span class="absolute left-0 text-amber-500/70 text-sm">📞</span>
          ...
          <!-- Kanan: Submit Button & Oauth Alignment -->
      ...
      <!-- Google OAuth & Back Link Row -->
      <div class="flex flex-col items-center gap-4 sm:flex-row sm:justify-between">
        <a href="#"
          class="flex h-10 w-full max-w-[260px] items-center justify-center gap-2 rounded-full border border-slate-700 bg-white transition hover:bg-slate-50">
        ...
        <p class="text-[11px] font-medium text-slate-600">
          Sudah punya akun? <a href="{{ route('login') }}" class="font-bold text-[#fec107] hover:underline">Login</a>
        </p>
      </div>
    </div>
