...
import { Search, X, Plus, Trash2, ChevronDown, GripVertical, Layers, Lock, Copy, Eye, ChevronLeft, ChevronRight, Building } from 'lucide-vue-next';
...
const activeTab     = ref('fields'); // 'fields' | 'gallery'
...
const selectedGalleryToAdd = ref('');
...
onMounted(() => {
    ...
    if (props.assetTypes && props.assetTypes.length > 0) {
        const lastSelected = localStorage.getItem('last_selected_asset_type');
...
function selectAssetType(id) {
    if (id) {
        ...
        localStorage.setItem('last_selected_asset_type', id);
    }
    isDropdownOpen.value = false;
}
...
function loadDraft() {
    ...
    draftGallery.value = JSON.parse(JSON.stringify(
        galleryScope.value === 'asset' ? (t.gallery_categories || []) : (t.unit_gallery_categories || [])
    )).map(cat => ({ ...cat, key: 'cat_' + cat.id })); // Add unique key for draggable
    activeEditIndex.value = null;
}
...
function saveFields() {
    if (!selectedType.value) return;
    router.put(route('admin.template-aset.fields', selectedType.value.id), {
...
function saveGallery() {
    ...
    router.put(route('admin.template-aset.gallery', selectedType.value.id), {
...
<template>
    <Head title="Konfigurasi Aset - Admin Panel" />

    <DashboardLayout role="Admin" title="Konfigurasi Aset" description="Kelola konfigurasi field informasi dan kategori galeri wajib per tipe aset." no-padding>
        <template #leftAction>
        ...
        <div class="flex justify-center w-full min-h-[60vh]">
            ...
            <div v-if="!selectedType" class="w-full max-w-4xl flex flex-col items-center justify-center text-slate-400 gap-3 py-32">
                ...
                <p class="text-xs">untuk mengatur konfigurasi field dan kategori galeri wajib</p>
            </div>
            ...
            <div v-else class="flex flex-col lg:flex-row w-full items-start">
                ...
                <div class="w-full lg:w-[340px] shrink-0 bg-white border-r border-slate-200 flex flex-col h-[calc(100vh-121px)] lg:h-[calc(100vh-61px)] sticky top-[121px] lg:top-[61px] z-20 shadow-[4px_0_24px_-15px_rgba(0,0,0,0.1)]">
                    ...
                    <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                        <div class="flex items-center gap-1 bg-slate-200/60 p-1 rounded-xl text-xs w-full shadow-inner">
                            <button
                                v-for="tab in ['fields', 'gallery']"
                                :key="tab"
                                @click="activeTab = tab"
                                :class="['flex-1 py-2.5 rounded-lg font-bold transition capitalize text-center', activeTab === tab ? 'bg-primary text-[#0A2540] shadow-sm' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50']"
                            >
                                {{ tab === 'fields' ? 'Spesifikasi Aset' : 'Kategori Galeri' }}
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="flex-1 overflow-y-auto bg-slate-50/30 flex flex-col relative [&::-webkit-scrollbar]:hidden" style="scrollbar-width: none;">
                        ...
                        <!-- ── TAB: Field Informasi ────────────────────────── -->
                        <div v-if="activeTab === 'fields'" class="flex flex-col h-full">

                            <!-- Scope toggle & List Header -->
                            <div class="px-4 pt-4 pb-3 flex items-center justify-between sticky top-0 bg-slate-50/95 backdrop-blur z-20 border-b border-slate-200 shadow-sm gap-2">
                                <h3 class="font-bold text-slate-800 text-[13px] whitespace-nowrap">Field</h3>
                                <div class="flex items-center gap-2">
                                    ...
                                    <div class="flex items-center gap-1 bg-white p-1 rounded-lg border border-slate-200/80 text-[10px] shadow-sm">
                                        <button
                                            ...
                                            :class="[
                                                'px-3 py-1 rounded-md font-bold transition capitalize',
                                                fieldScope === s ? 'bg-primary text-white' : 'text-slate-500 hover:bg-slate-50',
                        ...
                        <!-- ── TAB: Galeri Wajib ──────────────────────────── -->
                        <div v-if="activeTab === 'gallery'" class="flex flex-col h-full">
                            <!-- Scope toggle & Header -->
                            <div class="px-4 pt-4 pb-3 flex items-center justify-between sticky top-0 bg-slate-50/95 backdrop-blur z-20 border-b border-slate-200 shadow-sm gap-2">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <div class="flex items-center gap-1 bg-white p-1 rounded-lg border border-slate-200/80 text-[10px] shadow-sm">
                                        <button
                                            ...
                                            :class="[
                                                'px-3 py-1 rounded-md font-bold transition capitalize',
                                                galleryScope === s ? 'bg-primary text-white' : 'text-slate-500 hover:bg-slate-50',
                                                s === 'unit' && !selectedType.allow_units ? 'opacity-40 cursor-not-allowed' : ''
                                            ]"
                                        >
                                            {{ s === 'asset' ? 'Aset' : 'Unit' }}
                            ...
                            <div class="flex-1 p-4 lg:p-5">
                                <draggable v-model="draftGallery" item-key="key" handle=".drag-handle" :animation="200" class="space-y-1 pb-2">
                                    <template #item="{ element: cat, index: idx }">
                                        <div class="rounded-lg transition-colors border" :class="activeEditIndex === idx ? 'bg-slate-100 border-slate-200' : 'border-slate-200/60 hover:bg-slate-50 bg-white'">
                                            <div class="flex items-center px-3 py-2.5 gap-2.5 cursor-pointer group select-none" @click.stop="openSettings(idx, $event)" @contextmenu.prevent.stop="openSettings(idx, $event)">
                                                <div class="drag-handle text-slate-300 group-hover:text-slate-400 cursor-grab active:cursor-grabbing p-0.5 -ml-1 transition-colors" title="Seret untuk memindahkan urutan" @click.stop>
                                                    <GripVertical :size="14" />
                                                </div>
                            ...
                            <!-- Footer Save Button -->
                            <div class="p-5 bg-white border-t border-slate-200 mt-auto sticky bottom-0 z-20 shadow-[0_-15px_30px_-10px_rgba(0,0,0,0.08)]">
                                <button @click="saveGallery" class="w-full rounded-lg bg-[#FFC000] px-5 py-3 text-sm font-bold text-[#0A2540] hover:bg-amber-400 transition shadow-sm flex items-center justify-center gap-2">
                                    Simpan Konfigurasi Galeri
                                </button>
                            </div>
                        </div>
                ...
                <div class="flex-1 w-full bg-slate-50/70 p-6 lg:p-12 relative">
                    ...
                    <div v-if="activeTab === 'fields'" class="mt-8 max-w-4xl mx-auto pb-20">
                        <!-- MOCK WRAPPER TOP -->
                    ...
                    <div v-if="activeTab === 'gallery'" class="mt-8 max-w-4xl mx-auto pb-20">
                        <div class="space-y-8">

                            <!-- Bawaan: Foto Sampul Utama -->
                            <div class="pointer-events-none">
                                <div class="flex items-baseline gap-2 mb-2">
                                    <label class="text-base font-bold text-[#0A2540]">Foto Sampul Utama <span class="text-rose-500">*</span></label>
                                    <span class="text-xs text-slate-500 font-medium">(Akan ditampilkan paling depan)</span>
                                </div>
                                <div class="w-full rounded-2xl border-2 border-dashed border-slate-300 bg-white flex flex-col items-center justify-center py-10 shadow-sm">
                                    ...
                                    <p class="text-xs text-slate-400 font-medium">Format didukung: JPG, JPEG, PNG</p>
                                </div>
                            </div>

                            <!-- Dynamic List from draftGallery -->
                            <div v-for="catId in draftGallery" :key="catId" class="pointer-events-none transition-all duration-300">
                                <template v-for="cat in galleryCategories" :key="'g-'+cat.id">
                                    <div v-if="cat.id === catId">
                                        <div class="flex items-baseline gap-2 mb-2">
                                            <label class="text-base font-bold text-[#0A2540]">{{ cat.name }} <span class="text-rose-500">*</span></label>
                                            <span class="text-xs text-slate-500 font-medium">Pastikan area/fasilitas {{ cat.name.toLowerCase() }} terlihat dengan jelas.</span>
                                        </div>
                                        <div class="w-full rounded-2xl border-2 border-dashed border-slate-300 bg-white flex flex-col items-center justify-center py-10 shadow-sm">
                                            <div class="relative w-16 h-16 mb-4 flex items-center justify-center">
                                                <div class="absolute inset-0 bg-white border border-slate-200 rounded-lg shadow-sm rotate-6"></div>
    ...
    >
        <div v-if="activeEditIndex !== null" :key="activeEditIndex" class="fixed inset-0 lg:inset-auto z-[100] flex items-center justify-center lg:block p-4 lg:p-0 pointer-events-none popover-desktop lg:origin-left" :style="{ '--popover-top': popoverTop + 'px', '--popover-left': popoverLeft + 'px' }">
            ...
            <!-- Popover Content -->
            <div class="relative bg-white rounded-xl shadow-[0_10px_30px_-5px_rgba(0,0,0,0.15)] border border-slate-200 w-full max-w-[320px] flex flex-col max-h-[90vh] lg:max-h-[calc(100vh-80px)] pointer-events-auto" :class="activeTab === 'fields' ? 'overflow-hidden' : 'overflow-visible'" @click.stop>
                <div class="px-4 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 rounded-t-xl">
                    <h3 class="font-bold text-slate-700 text-sm">Pengaturan {{ activeTab === 'fields' ? 'Field' : 'Galeri' }}</h3>
                    <button @click="activeEditIndex = null" class="text-slate-400 hover:text-rose-500 transition">
                        <X :size="16" />
                    </button>
                </div>

                <div class="p-4 space-y-4" :class="activeTab === 'fields' ? 'overflow-y-auto' : 'overflow-visible'">
                    <template v-if="activeTab === 'fields' && draftFields[activeEditIndex]">
                    ...
                    <template v-if="activeTab === 'gallery' && draftGallery[activeEditIndex]">
                        <div class="relative mt-2">
                            <label class="absolute -top-2 left-2 px-1 text-[10px] font-bold text-slate-400 z-10 bg-white">Pilih Kategori Galeri</label>
                            <SearchableSelect 
                        ...
                        <div class="space-y-2 pt-4">
                            <label class="flex items-center gap-2.5 cursor-pointer">
                                <input type="checkbox" v-model="draftGallery[activeEditIndex].is_mandatory" class="rounded border-slate-300 w-3.5 h-3.5 text-slate-600 focus:ring-0">
                                <span class="text-xs font-semibold text-slate-600">Wajib Diupload</span>
                            </label>
                        </div>
                    </template>
                </div>
                ...
                <div class="px-3 py-2 flex flex-col gap-1.5 bg-white rounded-b-xl border-t border-slate-50">
                    <button v-if="activeTab === 'fields'" @click="duplicateField(activeEditIndex); activeEditIndex = draftFields.length - 1" class="w-full flex justify-center items-center gap-1.5 py-1.5 rounded-lg border border-slate-200 bg-white text-slate-600 font-semibold text-[11px] hover:bg-slate-50 transition">
                        <Copy :size="12" /> Duplicate Field
                    </button>
                    <button @click="activeTab === 'fields' ? removeField(activeEditIndex) : removeGallery(activeEditIndex)" class="w-full flex justify-center items-center gap-1.5 py-1.5 rounded-lg border border-rose-200 bg-white text-rose-500 font-semibold text-[11px] hover:bg-rose-50 transition">
                        <Trash2 :size="12" /> Delete {{ activeTab === 'fields' ? 'Field' : 'Galeri' }}
                    </button>
                </div>
            </div>
        </div>
