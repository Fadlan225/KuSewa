...
import { Search, X, Plus, Trash2, ChevronDown, GripVertical, ChevronLeft, ChevronRight, Building } from 'lucide-vue-next';
...
<template>
                            ...
                            <div class="px-4 pt-3 pb-3 flex flex-col gap-3 sticky top-0 bg-slate-50/95 backdrop-blur z-20 border-b border-slate-200 shadow-sm">
                                ...
                                <div class="flex items-center w-full bg-white p-1 rounded-lg border border-slate-200/80 text-[11px] shadow-sm">
                                    <button
                                        ...
                                        :class="[
                                            galleryScope === s ? 'bg-primary text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50',
    ...
    >
        <div v-if="activeEditIndex !== null" :key="activeEditIndex" class="fixed inset-0 lg:inset-auto z-[100] flex items-center justify-center lg:block p-4 lg:p-0 pointer-events-none popover-desktop lg:origin-left" :style="{ '--popover-top': popoverTop + 'px', '--popover-left': popoverLeft + 'px' }">
            ...
            <!-- Popover Content -->
            <div class="relative bg-white rounded-xl shadow-[0_10px_30px_-5px_rgba(0,0,0,0.15)] border border-slate-200 w-full max-w-[320px] flex flex-col max-h-[90vh] lg:max-h-[calc(100vh-80px)] pointer-events-auto overflow-visible" @click.stop>
                <div class="px-4 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 rounded-t-xl">
                    <h3 class="font-bold text-slate-700 text-sm">Pengaturan Galeri</h3>
                ...
                <div class="px-3 py-2 flex flex-col gap-1.5 bg-white rounded-b-xl border-t border-slate-50">
                    <button @click="removeGallery(activeEditIndex)" class="w-full flex justify-center items-center gap-1.5 py-1.5 rounded-lg border border-rose-200 bg-white text-rose-500 font-semibold text-[11px] hover:bg-rose-50 transition">
                        <Trash2 :size="12" /> Delete Galeri
                    </button>
                </div>
            </div>
        </div>
