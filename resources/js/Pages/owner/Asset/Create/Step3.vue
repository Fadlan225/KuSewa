        ...
        <div class="mb-6 space-y-6">
            <template v-for="field in detailFields" :key="field.key">
                <div>
                    ...
                    <label v-else-if="field.type === 'checkbox'" class="flex items-center cursor-pointer relative w-fit">
                        <input type="checkbox" v-model="form.detail[field.key]" class="sr-only peer">
                        <div class="w-11 h-6 bg-slate-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-[#FFC000]/50 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#FFC000]"></div>
                        <span class="ml-3 text-sm font-medium text-slate-700">Ya</span>
                    </label>
                    ...
                    <!-- Default: Text, Number -->
                    <input v-else :type="field.type" :step="field.type === 'number' ? 'any' : undefined" v-model="form.detail[field.key]" :placeholder="field.label" class="w-full text-sm px-4 py-2.5 rounded-md border border-slate-300 focus:outline-none focus:ring-2 focus:ring-[#FFC000] focus:border-transparent transition" :required="field.required" />
                </div>
            </template>
        </div>
    </template>
                        ...
                        <template v-if="unitDetailFields.length > 0">
                            <template v-for="field in unitDetailFields" :key="field.key">
                                <div class="mb-8 pb-8 border-b border-slate-200/70">
                                    ...
                                    <!-- Type: Radio (Pill Style) -->
                                    <div v-if="field.type === 'radio'" class="flex items-center w-full bg-white border border-slate-200 rounded-lg overflow-hidden flex-wrap sm:flex-nowrap">
                                        <label v-for="(opt, idx) in field.options" :key="opt" class="flex-1 text-center py-3 cursor-pointer transition-colors w-full sm:w-auto" :class="[unit.detail[field.key] === opt ? 'bg-[#FFF8E6] text-[#0A2540] font-bold' : 'text-slate-500 hover:bg-slate-50', idx < field.options.length - 1 ? 'sm:border-r border-b sm:border-b-0 border-slate-200' : '']">
                                            <input type="radio" :name="`unit_${unitIndex}_${field.key}`" :value="opt" v-model="unit.detail[field.key]" class="hidden" />
                                            {{ opt }}
                                        </label>
                                    </div>

                                    <!-- Type: Select -->
                                    <select v-else-if="field.type === 'select'" v-model="unit.detail[field.key]" class="w-full text-sm px-4 py-2.5 rounded-md border border-slate-300 focus:outline-none focus:ring-2 focus:ring-[#FFC000] focus:border-transparent transition" :required="field.required">
                                        <option value="" disabled>Pilih...</option>
                                        <option v-for="opt in field.options" :key="opt" :value="opt">{{ opt }}</option>
                                    </select>
                                    ...
                                    <!-- Type: Checkbox (Toggle style) -->
                                    <label v-else-if="field.type === 'checkbox'" class="flex items-center cursor-pointer relative w-fit">
                                        <input type="checkbox" v-model="unit.detail[field.key]" class="sr-only peer">
                                        <div class="w-11 h-6 bg-slate-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-[#FFC000]/50 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#FFC000]"></div>
                                        <span class="ml-3 text-sm font-medium text-slate-700">Ya</span>
                                    </label>

                                    <!-- Type: Time -->
                                    <input v-else-if="field.type === 'time'" type="time" v-model="unit.detail[field.key]" class="w-full text-sm px-4 py-2.5 rounded-md border border-slate-300 focus:outline-none focus:ring-2 focus:ring-[#FFC000] focus:border-transparent transition" :required="field.required" />

                                    <!-- Default: Text, Number -->
                                    <input v-else :type="field.type" :step="field.type === 'number' ? 'any' : undefined" v-model="unit.detail[field.key]" :placeholder="field.label" class="w-full text-sm px-4 py-2.5 rounded-md border border-slate-300 focus:outline-none focus:ring-2 focus:ring-[#FFC000] focus:border-transparent transition" :required="field.required" />
                                </div>
                            </template>
                        </template>
