...
<template>
    <Head title="Kelola Aset Saya" />

    <DashboardLayout
        title="Kelola Aset Saya"
        description="Pantau status keterisian, harga, dan manajemen unit yang Anda sewakan."
        role="Owner"
    >
        <template #action>
            <Link
        ...
        <div class="space-y-6 mt-6">
                ...
                <div class="bg-white border border-slate-200/60 shadow-sm rounded-xl p-4 md:p-5 space-y-4 relative z-20">
                    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        ...
                        <div class="flex items-center gap-3 w-full lg:w-auto justify-between lg:justify-end flex-wrap sm:flex-nowrap">
                            ...
                            <div class="flex items-center bg-slate-100 p-1 rounded border border-slate-200/50">
                                <button
                                    @click="viewMode = 'grid'"
                                    :class="['p-2 rounded text-xs transition-all duration-200 flex items-center justify-center', viewMode === 'grid' ? 'bg-[#FFC000] text-[#0A2540] shadow-sm font-bold' : 'text-slate-400 hover:text-slate-600']"
                                    title="Tampilan Grid"
                                >
                                    <Grid3X3 class="" />
                                </button>
                                <button
                                    ...
                                    :class="['p-2 rounded text-xs transition-all duration-200 flex items-center justify-center', viewMode === 'table' ? 'bg-[#FFC000] text-[#0A2540] shadow-sm font-bold' : 'text-slate-400 hover:text-slate-600']"
                    ...
                    <div class="flex items-center gap-2 overflow-x-auto pb-1 text-sm scrollbar-hide border-t border-slate-100 pt-3">
                        <button
                            ...
                            :class="[
                                'px-4 py-2 rounded font-semibold transition-all whitespace-nowrap border',
                ...
                <div v-if="filteredProperties.length === 0" class="flex flex-col items-center justify-center py-16 px-4 text-center">
                    <EmptyStateIcon class="w-40 sm:w-48 h-auto mb-5 mx-auto opacity-70 drop-shadow-sm" />
                ...
                <div v-if="props.properties?.last_page > 1" class="flex items-center justify-center gap-1 pt-2">
                    <Link
                        ...
                        :class="[
                            'text-xs font-bold px-3 py-1.5 rounded transition',
                            link.active ? 'bg-[#FFC000] text-[#0A2540] shadow-sm' : 'text-slate-500 hover:bg-slate-100',
                        ]"
                    />
                </div>
