...
import { Loader2, RefreshCw, Clock } from 'lucide-vue-next';
  ...
  <AppLayout hide-navbar>
    ...
    <div class="min-h-screen bg-[#F8F9FA] font-sans text-gray-700 py-6 px-4 md:px-8">
      <div class="max-w-3xl mx-auto">
        ...
        <header class="mb-6">
          <div class="mb-4 md:hidden" v-if="hasRealUnread">
              <button
                @click="markAllAsRead"
          ...
          <!-- FILTERS -->
          <div class="flex overflow-x-auto gap-2 pt-2 pb-2 px-1 hide-scrollbar -mx-1">
            <button
          ...
          <div v-for="(notifs, dateGroup) in groupedNotifications" :key="dateGroup">
            ...
            <div class="flex flex-col gap-3">
              <component
                ...
                :key="notif.id"
                :class="[
                  'relative flex flex-col md:flex-row gap-4 p-4 md:p-5 rounded-md border transition-all duration-300 md:items-center',
                  notif.data?.action_url ? 'cursor-pointer' : '',
                  notif.read_at ? 'bg-gray-50/50 border-gray-100' : 'bg-white border-gray-200 shadow-sm hover:border-gray-300'
                ]"
              >
                ...
                <!-- Icon / Logo -->
                <div class="shrink-0 flex items-start md:items-center">
                  <div :class="[
                    'w-14 h-14 rounded-full flex items-center justify-center bg-white shadow-sm border border-gray-100',
                    notif.read_at ? 'opacity-70' : getStatusColor(getNotifMeta(notif).status)
                  ]">
                    <component v-if="notif.data?.icon" :is="LucideIcons[notif.data.icon]" class="w-7 h-7" :class="notif.read_at ? 'text-gray-400' : ''" />
                    <img v-else src="/kitasewa-logo.png" alt="KitaSewa" class="w-8 h-8 object-contain" :class="notif.read_at ? 'grayscale opacity-60' : ''" />
                  </div>
                </div>
                ...
                <div class="flex-1 flex flex-col justify-center">
                  ...
                  <span class="text-xs font-medium text-gray-500 flex items-center gap-1">
                    <Clock class="w-3 h-3" /> {{ formatDateTime(notif.created_at) }}
                  </span>
                </div>
